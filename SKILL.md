...
The form package has `InputField`, `TextArea`, `SelectInput`, `MultiSelectInput`, `RadioInput`, and `InlineRadioGroup`. The control package has `Button` and `RangeSlider`. The display package has `Accordion`, `Alert`, `CloakLoading`, `LoadingOverlay`, `Modal`, `Sidebar`, `Tag`, and `Toast`.

Most tasks touch one or two packages, so read only the `.templ` files that apply. Each component file defines the component and its settings struct together.
...
- `README.md` in the module root — the library overview, styling, and toolset docs.
...
- Call functions with the `Func` suffix (full call with parentheses, e.g. `OnClickFunc: "myFunction('hi')"`) or the `FuncName` suffix (bare name; the component passes the parameters).
...
- Import the packages plainly. The package names are already prefixed: `uiForm`, `uiControl`, `uiDisplay`, `uiImport`, `uiToolset`. No import alias is needed.
