...
const results = JSON.parse(
  readFileSync(new URL("../performance-results.json", import.meta.url), "utf8"),
);
const budgets = yaml.load(
  readFileSync(new URL("../golden.yaml", import.meta.url), "utf8"),
);
...
for (const [metric, elapsedMs] of Object.entries(results)) {
  ...
  if (!budget) {
    console.log(
      `WARN ${metric}: ${elapsedMs}ms — no golden budget recorded yet`,
    );
    continue;
  }
  ...
  if (budget.fatal_ms && elapsedMs >= budget.fatal_ms) {
    console.error(
      `FATAL ${metric}: ${elapsedMs}ms >= ${budget.fatal_ms}ms budget`,
    );
    fatalReached = true;
    continue;
  }
  if (elapsedMs >= warnAtMs) {
    console.log(
      `WARNING ${metric}: ${elapsedMs}ms >= ${Math.round(warnAtMs)}ms (run still passes)`,
    );
    continue;
  }
  ...
  console.log(
    `PASS ${metric}: ${elapsedMs}ms (baseline ${budget.baseline_p50_ms}ms)`,
  );
}
